// #[allow(dead_code)]
// use super::{dp, meet_in_the_middle};
...
// {
//     if n <= 40 {
//         meet_in_the_middle::knapsack_half_enumerate(n, c, w, v)
//     } else if !n.overflowing_mul(c.as_()).1 {
//         let c = c.as_();
//         let w: Vec<_> = w.iter().map(|wi| wi.as_()).collect();
//         dp::knapsack_dp_weight(n, c, &w, &v)[n][c]
//     } else {
//         let v: Vec<_> = v.iter().map(|vi| vi.as_()).collect();
//         V::from(dp::knapsack_dp_value_solve(n, c, &w, &v)).unwrap()
//     }
// }
...
//         ];
//         assert_eq!(knapsack(n, c, &w, &v), 3673016420usize);
//     }
